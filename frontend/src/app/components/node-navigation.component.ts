...
import { Interpolator, ContinuousBezierInterpolator, LinearInterpolator, ContinuousInterpolator } from '../util/animate';
import { rotatePoint, Vector } from '../util/vector';
import { BezierCurve } from '../util/bezier';
import { NavigationNode } from '../model/node-navigation.model';
...
export class NodeNavigationComponent implements OnInit, AfterViewInit {
  ...
  renderer: ContinuousInterpolator;
  ...
  public set current(node: NavigationNode) {
    this._current = node;
    this.navigationService.navigate(node.name);
  }
  ...
  constructor(private navigationService: NodeNavigationService, private router: Router) {
    this.renderer = new ContinuousInterpolator(() => {
      ...
      for (const line of this.nodeLines) {
      ...
      for (const view of this.nodeViews) {
        ...
        this.context.fillStyle = this.groupView(view) ? this.Background : this.Foreground;
        this.context.strokeStyle = this.groupView(view) ? this.Foreground : this.Background;
        ...
        const below = new Vector(resolved.x, resolved.y + view.radius / 4);
        const shade = this.groupView(view) ? 255 : 128;
        this.context.fillStyle = `rgba(${shade}, ${shade}, ${shade}, ${view.expanded * view.opacity})`;
        if (this.groupView(view)) {
          const y = resolved.y + view.radius / 3;
          const x = resolved.x;
  ...
  ngAfterViewInit() {
    ...
    this.navigationService.root().subscribe((node) => {
      this.update(node);
    });
  ...
  public update(node: NavigationNode) {
    ...
    for (const view of this.backgroundViews) {
      view.interpolator.stop();
    }
    this.nodeViews = [];
    this.nodeLines = [];
  ...
  public onMouseClick(x: number, y: number) {
    if (this.active) {
      const mouse = new Vector(x * window.devicePixelRatio, y * window.devicePixelRatio);
      for (const view of this.nodeViews) {
  ...
  public findPrecedent() {
    let max = 0;
    for (const view of this.nodeViews) {
      const sep = view.resolved.subtract(this.mouse);
      const dist = sep.norm();
      if (dist < view.radius) {
        this.precedent = view;
  ...
  public adjustView(view: NodeView) {
    const mouseSeperation = view.resolved.subtract(this.mouse);
    const mouseDist = mouseSeperation.norm();
    this.findPrecedent();
    ...
    if (view !== this.precedent) {
      ...
      for (const compare of this.nodeViews) {
        if (compare !== view) {
          const resolvedSeperation = view.resolved.subtract(compare.resolved);
          const resolvedDist = resolvedSeperation.norm();
  ...
  public setupView(node: NavigationNode, center: Vector, radius: number = 50): NodeView {
    ...
    view.node = node;
    const curve = this.randomCurveInRange(view.position,
  ...
  public setupViewChildren(view: NodeView): NodeView[] {
    ...
    for (const child of view.node.children) {
      const point = view.position.add(offset);
      const childView = this.setupView(child, point, view.radius / 2);
  ...
  public randomCurveInRange(start: Vector, lower: Vector, upper: Vector): BezierCurve {
    const controls = [new Vector(...start.data)];
    ...
    const change = first.subtract(start);
  ...
  public randomCurve(start: Vector) {
    return this.randomCurveInRange(
      start,
      new Vector(-this.width / 2, -this.height / 2),
      new Vector(this.width * 3 / 2, this.height * 3 / 2)
    );
  }

  public extendCurve(start: Vector, tangent: Vector): BezierCurve {
    const first = new Vector(...start.data);
    const controls = [first, first.add(tangent.unit().multiply(10))];
    for (let i = 0; i < Math.floor(Math.random() * 2) + 1; i++) {
  ...
  public retractPage() {
    ...
    if (find == null) {
      throw new Error('failed to retract navigation page');
    }
  ...
  public expandGroup(view: NodeView, retract: boolean = false) {
    ...
    const finished = () => {
      ...
      const mid = new Vector(this.width / 2, this.height / 2);
      view.pull = view.resolved.subtract(mid);
      view.position = mid;
      view.interpolator = new ContinuousBezierInterpolator(
        view.time,
        (target.interpolator as ContinuousBezierInterpolator).curve,
        (point: Vector) => {
          view.position = point;
          this.adjustView(view);
        }
      ...
      this.nodeLines = [];
      const childViews = this.setupViewChildren(target);
      ...
      for (const child of this.nodeViews) {
        child.opacity = 1.0;
        child.interpolator.start();
      }
      this.precedent = view;
    ...
    for (const child of this.nodeViews) {
      ...
      if (child.node === this.current) {
        ...
        complete = () => {
          const index = this.nodeViews.indexOf(child);
      ...
      const initial = new Vector(...child.resolved.data);
      const radius = child.radius;
      const dir = new Vector(view.resolved.x - child.resolved.x, view.resolved.y - child.resolved.y);
